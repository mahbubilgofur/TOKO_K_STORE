...
class Produk extends CI_Controller
{
    ...
    public function Inputproduk()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            // Mengatur nilai gambar ke '0' jika tidak ada gambar yang diunggah
            while (count($gambar_names) < 5) {
                $gambar_names[] = '0';
            }
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            for ($i = 1; $i <= 5; $i++) {
                ...
                if (!empty($_FILES[$field_name]['name'])) {
                    ...
                    // Use try-catch to handle upload errors
                    try {
                        if ($this->upload->do_upload($field_name)) {
                            $data = $this->upload->data();
                            $gambar_name = $data['file_name'];
                            $gambar_names[] = $gambar_name;
                        } else {
                            throw new Exception($this->upload->display_errors());
                        }
                    } catch (Exception $e) {
                        $this->session->set_flashdata('error', 'Failed to upload ' . $field_name . ': ' . $e->getMessage());
                        redirect(base_url('produk/'));
                    }
                } else {
                    // Use $this->input->post() for getting post data
